...
async function initSite() {
    ...
    if (isDetailPage && propertyId) {
        // A página imovel.html já carrega imovel.js, mas mantemos compatibilidade se necessário
...
function applySiteSettings(config) {
    ...
    // O fundo do Hero já é gerenciado pelo CSS com suporte a dark mode, 
    // mas permitimos override se houver URL nas configs.
    const heroSection = document.querySelector('header');
    if (heroSection && config.hero_imagem_url && window.innerWidth > 768) {
        heroSection.style.backgroundImage = `url('${config.hero_imagem_url}')`;
    }
...
async function loadHomeProperties() {
    const container = document.getElementById('lista-imveis') || document.getElementById('lista-imoveis');
    if (!container) return;
